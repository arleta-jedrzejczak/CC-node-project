...
exports.send = function (name, email, password) {
    var token = jwt.sign({ name: name, email: email, password: password }, process.env.JWT_KEY, {
        expiresIn: "15m",
    });
    mg.messages().send({
...
exports.activateAccount = function (req, res, next) {
    ...
    if (token) {
        jwt.verify(token, process.env.JWT_KEY, function (err, decodedToken) {
            ...
            var name = decodedToken.name, email = decodedToken.email, password = decodedToken.password;
